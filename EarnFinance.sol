...
contract EarnFinance is Context, IBEP20, Ownable {
    ...
    string private _name = "Earn Finance";
    ...
    uint256 internal _tokenTotal = 5e28;
    ...
    bool public canChangeTransFee = true;
    ...
    constructor(address marketing, address rewards) {
    ...
    function updateMarketingAddress(address marketing) public onlyOwner {
        require(marketing != marketingAddress, "EARN: Account is already added.");
        marketingAddress = marketing;
    }
    
    function updateLpTokenRecipient(address recipient) public onlyOwner {
        require(recipient != recipient, "EARN: Account is already added.");
        lpTokenRecipient = recipient;
    }
    
    function addRemovePair(address pairAddress, bool value) public onlyOwner {
        require(isPair[pairAddress] != value, "EARN: Account is already added.");
    ...
    function excludeFromReward(address account) external onlyOwner {
        require(account != address(uniswapV2Router), "EARN: Uniswap router cannot be excluded.");
        require(account != address(this), "EARN: The contract it self cannot be excluded");
        require(!_isExcluded[account], "EARN: Account is already excluded");
    ...
    function includeForReward(address account) external onlyOwner {
        require(_isExcluded[account], "EARN: Account is already included");
    ...
    function _transfer(address sender, address recipient, uint256 amount) private {
        ...
        require(amount > 0, "EARN: Transfer amount must be greater than zero");
        require(tradingEnabled || isExcludedFromFee[sender] || isExcludedFromFee[recipient], "EARN: Trading is locked before presale..");
        require(!isBlacklisted[sender], "EARN: You are blacklisted..");
        ...
        if (!isExcludedFromFee[sender] && !isExcludedFromFee[recipient]) {
            ...
            if (isPair[sender]) {
                require(amount <= maxBuyAmount, "EARN: Buy amount exceeds the maximum buy amount");
                transferAmount = collectFeeOnBuy(sender,amount,rate);
            }
            if (isPair[recipient]) {
                require(amount <= maxSellAmount, "EARN: Sell amount exceeds the maximum sell amount");
    ...
    function addRemoveFromBlackList(address botAddress, bool value) public onlyOwner {
        require(isContract(botAddress), "EARN: You can blacklit only bot not an user...");
    ...
    function addRemoveContract(address contAdd, bool value) public onlyOwner {
        require(isThisContractCanTransfer[contAdd] != value, "EARN: already same value...");
    ...
    function excludedOrEncludedFromFee(address account, bool value) public onlyOwner {
        require(isExcludedFromFee[account] != value, "EARN: Account is already excluded");
    ...
    function updateBuySellMaxAmount(uint256 maxBuy, uint256 maxSell) public onlyOwner {
        require(maxBuy >= totalSupply().mul(1).div(10000) && maxSell >= totalSupply().mul(1).div(10000), "EARN: Please enter 0.001% of total supply or more..");
        maxBuyAmount = maxBuy;
    ...
    function updateTransferFees(uint256 tax, uint256 liquidity, uint256 marketing, uint256 rewards, uint256 burn) public onlyOwner {
        require(tax <= 10 && liquidity <= 10 && marketing <= 10 && rewards <= 10 && burn <= 10, "EARN: Enter a valid number..");
        _taxFee = tax;
    ...
    function enableTrading() external onlyOwner {
        require(!tradingEnabled, "EARN: Already enabled..");
        tradingEnabled = true;
        emit TradingEnabled(true);
    }

    function airdrop(address[] memory recipient, uint256[] memory amount) public onlyOwner {
        require(recipient.length == amount.length, "EARN: Incorrect recipient and amount length.");
        
        for (uint256 i = 0; i < recipient.length; i++) {
            approve(tx.origin, amount[i]);
            transferFrom(tx.origin, recipient[i], amount[i]);
        }
    }

    function stopAllTransferFees() public onlyOwner {
        require(canChangeTransFee, "EARN: Already stopped !");
        canChangeTransFee = false;
        _taxFee = 0;
        _liquidityFee = 0;
        _marketingFee = 0;
        _rewardsFee = 0;
